...
void write_tr(char argv[]){
	...
	//cout<<"hi";
	//cout<<"\nooop"<<tr_length<<endl;
	stream <<setfill('0') << setw(6)<<hex<<(tr_length&0xffffff);
...
string displace(string source,string dest,int format){
		...
		if(format==3){
			if(y<0)
			{	res<<setfill('0') << setw(4)<<hex<<(y&0xffff);
...
int assemble(string opcode,string operand_m[],string next){
	...
	if(s_o>=0){
		...
		if(OPTAB[s_o][2].compare("1")==0){//format1
			...
			//string s = "0xA";
   			...
   			ss >> n;
    			bitset<4> ni(n);
			bitset<4> xbpe(0000);
			//cout<<"\n aaaaaaaaaaaaaaaa"<<xbpe<<endl<<endl;
			...
			if(operand_m[0].compare("#")==0){
				...
				//cout<<si;
				if(si>=0){
					disp=displace("0",SYMTAB[si][1],3);//change 0 to next for sicxe
				...
				//ni.set(0);ni.set(0);//change 1 to next for sicxe
				//cout<<"ni"<<ni;
		...
		if(opcode.compare("WORD")==0){
			...
			no>>n;
			hh<<setfill('0') << setw(6)<<hex<<(n&0xffffff);
			OBJ_CODE=hh.str();
			cout<<"hhhhhhhhh";
			return 1;
		}
